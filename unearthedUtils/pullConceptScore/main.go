...
func getExistingCids(yid string) ([]*Cid, error) {
	var cids []*Cid
	...
	queryExpression := "yid = :yid"
	var expressionMap = map[string]interface{}{}
	expressionMap[":yid"] = yid

	fullRecords, err := dynamo.Query("HLDA_UnearthedDocumentConcepts_v4", queryExpression, expressionMap)
	if err != nil {
		log.WithError(err).Error("")
		return cids, err
	}
...
func main() {
	...
	key := "/Users/ziling/Desktop/yids_springer.txt"
	f, err := os.Open(key)
	if err != nil {
		log.Debugf(err.Error())
	}
	...
	close(sourceInChan)
	sourceOutChan := make(chan *Row, 8000000)

	var wg sync.WaitGroup
	for i := 0; i < 10; i++ {
		wg.Add(1)
		go func(id int, inChan chan string, outChan chan *Row, wg *sync.WaitGroup) {
			defer wg.Done()
			for yid := range inChan {
				cids, err := getExistingCids(yid)
				if err != nil {
					log.Debugf(yid)
					log.Debugf(err.Error())
				}
				...
				if cids != nil {
					for _, cid := range cids {
						...
						log.Infof("%s : %s : %s", yid, cid.Cid, cid.Score)
						outChan <- row
					}
				}
			}
	...
	flagFile := "/Users/ziling/Desktop/yid_cid_score.txt"
